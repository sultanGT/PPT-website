...
export default function PurchasePage(props) {//Reused code edited
  ...
  const { customer_purchase, loading, error } = purchaseDetails;//Reused code edited
...
//if payapal transaction is succesfull reload purchase with purchase ID
    if (!customer_purchase ||successPayment || successShipping || (customer_purchase && customer_purchase._id !== purchaseId)
    ) {
      ...
      if (!customer_purchase.purchase_confirmed) {//Reused code edited
        if (!window.paypal) {addPayPalScript();
        } else {setSdkReady(true);
        }
      }
    }
  }, [dispatch, purchaseId, sdkReady, successPayment, successShipping, customer_purchase]);//Reused code edited

  const PayPalHandler = (purchase_complete) => {dispatch(purchasePayPal(customer_purchase, purchase_complete));//Reused code edited
  };
  const shippingHandler = () => {dispatch(shippingPurchase(customer_purchase._id));//Reused code edited
  };
  ...
  return loading ? (<LoadingBox></LoadingBox>) : error ? (<MessageBox variant="danger">{error}</MessageBox>) : (
    <div className="pager">
      {customer_purchase.delivery_confirmed ? ( //selfcoded
            ...
            <h1>Order {customer_purchase._id}</h1>
        ...
        <div className="col-2">
          <ul>
            <li>
                ...
                <h2>Shipping</h2>
                <p>
                <strong>Name:   </strong>{customer_purchase.delivery_address.fullName} <br /> {/*edited*/}
                  <strong>Address:  </strong>{customer_purchase.delivery_address.address}<br />{/*edited*/}
                  <strong>City:   </strong>{customer_purchase.delivery_address.city}<br />{/*edited*/}
                  <strong>Post Code:   </strong>{customer_purchase.delivery_address.post_code}<br />{/*edited*/}
                  <strong>County:   </strong>{customer_purchase.delivery_address.county}<br />{/*edited*/}
                  <strong>Contact Number:   </strong>{customer_purchase.delivery_address.contactNumber}<br />  {/*selfcoded*/}
                </p>
                {customer_purchase.delivery_confirmed ? (<MessageBox variant="success">
                    Delivered at {customer_purchase.delivery_date}
                  </MessageBox>
                ) : (
            ...
            <li>
              <div className="container-box container-box-info">{/*styling self coded*/}
                <h2>Payment</h2>
                <p>
                  <strong>Method:</strong> {customer_purchase.purchase_method}{/*edited*/}
                </p>
                {customer_purchase.purchase_confirmed ? (<MessageBox variant="success">Paid at {customer_purchase.purchase_date}</MessageBox>//edited
                ) : (
            ...
            <ul>
              ...
              <li>
                <div className="row">
                  ...
                  <div>
                    <strong>£{customer_purchase.total_cost.toFixed(2)}</strong>{/* sedited */}
                  </div>
                </div>
              </li>
              {!customer_purchase.purchase_confirmed && (
                <li>{!sdkReady ? (<LoadingBox></LoadingBox>) : (
                    <>
                      ...
                      <PayPalButton amount={customer_purchase.total_cost} onSuccess={PayPalHandler}></PayPalButton>{/* edited */}
                    </>
                  )}
                </li>
              )}
              {userDetails.userCredentialsAdministrator && customer_purchase.purchase_confirmed && !customer_purchase.delivery_confirmed && (//edited
                <li>
