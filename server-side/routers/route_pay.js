...
const route_pay = express.Router();
...
const customer_order = new PPTOrder({orderProducts: req.body.orderProducts, deliveryAddress: req.body.deliveryAddress, paymentPPorS: req.body.paymentPPorS, itemsPrice: req.body.itemsPrice, deliveryPrice: req.body.deliveryPrice, taxPrice: req.body.taxPrice, totalPrice: req.body.totalPrice, pptuser: req.pptuser._id,});
...
route_pay.get('/:id', userCredentialsAuthenticated, expressAsyncHandler(async (req, res) => { const customer_order = await PPTOrder.findById(req.params.id);
if (customer_order) { res.send(customer_order); } else { res.status(404).send({ message: 'PPTOrder Has Not Been Found' });}}));
...
route_pay.delete('/:id', userCredentialsAuthenticated, userCredentialsAdministrator, expressAsyncHandler(async (req, res) => {const customer_order = await PPTOrder.findById(req.params.id);
...
route_pay.put('/:id/deliver', userCredentialsAuthenticated, userCredentialsAdministrator, expressAsyncHandler(async (req, res) => {
